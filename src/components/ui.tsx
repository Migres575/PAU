...
export const TimelineEventCard = ({ date, title, description, imageUrl, wikiUrl }: { date: string; title: string; description: string; imageUrl?: string; wikiUrl?: string }) => <article className="card border-l-4 border-rose-500"><div className="flex gap-3">{imageUrl && <SafeImage src={imageUrl} alt={title} className="w-20 h-20 rounded bg-stone-100 object-contain"/>}<div className="space-y-1"><p className="text-xs font-semibold text-rose-700">{date}</p><p className="text-base font-semibold text-slate-900">{description}</p>{wikiUrl && <a href={wikiUrl} target="_blank" rel="noreferrer" className="text-sm text-rose-700 underline underline-offset-2">Veure a Viquipèdia</a>}<p className="text-xs uppercase tracking-wide text-slate-500">Tema: {title}</p></div></div></article>;
